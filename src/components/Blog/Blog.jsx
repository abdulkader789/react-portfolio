...
const Blog = () => {
  ...
  useEffect(() => {
    fetch(
      "https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/@mdabdulkaderjoni"
    )
      .then((resp) => resp.json())
      .then((data) => {
        setBlogs(data.items);
  ...
  const sanitizeHTML = (html) => {
    ...
    return (
      tempElement.textContent.slice(0, 120) ||
      tempElement.innerText.slice(0, 120) ||
      ""
    );
  };
  ...
  return isLoading ? (
    ...
    <div className="min-h-screen bg-[#f4f5f5] dark:bg-inherit py-10 rounded-xl mb-10  shadow-md">
      <div className="py-5">
        <h2 className="text-center dark:text-white text-4xl md:text-5xl font-bold mb-5">
          My Blogs
        </h2>
      </div>
      ...
      <div className="flex flex-wrap justify-center items-center">
        {blogs.map((blog) => (
          <div key={blog.guid} className="">
            <a target="_blank" rel="noreferrer" href={blog.link}>
              <div className="rounded overflow-hidden  flex flex-col max-w-xl">
                {console.log(blog)}
